...
from network import ReCoNet, ReCoNetSD1, Vgg16
...
batch_size = 2
input_frame_num = 1
model_path = "./models/1-19_epoch_4_batchSize_2.pth"
LR = 1e-3
ALPHA = 1e5
BETA = 1e10
GAMMA = 1e-2
...
def train():
    ...
    teacher = ReCoNet(input_frame_num).to(device)
    student = ReCoNetSD1(input_frame_num).to(device)
    ...
    for epoch in range(epoch_start, epoch_end + 1):
        ...
        for img1, img2, flow, mask in batch_iterator:
            ...
            # Forward pass
            with torch.no_grad():
                feature_t1, _, _ = teacher(img1)
                feature_t2, _, _ = teacher(img2)
            _, feature_s1, feature_map1, styled_img1 = student(img1)
            _, feature_s2, feature_map2, styled_img2 = student(img2)
        ...
        # Save model
        torch.save(student.state_dict(), f"./models/SD1_epoch_{epoch}_batchSize_{batch_size}.pth")
