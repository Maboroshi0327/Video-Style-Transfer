...
from network import ReCoNetSD1, ReCoNetSD2, Vgg16
...
batch_size = 2
input_frame_num = 1
model_path = "./models/SD1_epoch_4_batchSize_2.pth"
LR = 1e-3
ALPHA = 1e5
BETA = 1e10
GAMMA = 1e-2
...
def train():
    ...
    teacher = ReCoNetSD1(input_frame_num).to(device)
    student = ReCoNetSD2(input_frame_num).to(device)
    teacher.load_state_dict(torch.load(model_path, weights_only=True), strict=True)
    student.load_state_dict(torch.load(model_path, weights_only=True), strict=False)

    # Optimizer and loss
    adam = optim.Adam(student.parameters(), lr=LR)
    ...
    for epoch in range(epoch_start, epoch_end + 1):
        ...
        for img1, img2, flow, mask in batch_iterator:
            ...
            # Forward pass
            with torch.no_grad():
                feature_t1, _, _, _ = teacher(img1)
                feature_t2, _, _, _ = teacher(img2)
            feature_s1, feature_map1, styled_img1 = student(img1)
            feature_s2, feature_map2, styled_img2 = student(img2)
            ...
            sd_loss = 0
            sd_loss += L2distance(feature_t1, feature_s1)
            sd_loss += L2distance(feature_t2, feature_s2)
            sd_loss *= 0.01 * BETA
        ...
        # Save model
        torch.save(student.state_dict(), f"./models/SD2_epoch_{epoch}_batchSize_{batch_size}.pth")
